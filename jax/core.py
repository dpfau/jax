...
def pp_eqns(eqns: Sequence[JaxprEqn],
  ...
  pp_eqns_ = map(pp_eqn, eqns)
  if source_info:
    l = max(i + len(s) for x in pp_eqns_ for i, s in x.lines)
...
def pp_jaxpr_eqn_range(jaxpr: Jaxpr, lo: int, hi: int,
  ...
  if len(eqns) == 0 and len(jaxpr.eqns) != 0:
    ...
    if lo != 0:
      pp_eqns_.append(pp('...'))
    pp_eqns_.extend(pp_eqns(eqns, source_info=source_info))
